...
export const TwoColumnContentLayout: React.FC<TwoColumnContentLayoutProps> = ({
  ...
  return (
    <div className={styles.pageContainer}>
      {useSimpleHeader ? (
        <SimplePageHeader title={title} rightComponent={headerRightComponent} />
      ) : (
        <PageHeader
          title={title}
          onBack={onBack}
      ...
      {fullWidthContent && <div className={styles.fullWidthWrapper}>{fullWidthContent}</div>}

      <div className={styles.layoutWrapper}>
        <div className={styles.layoutGrid} style={{ gridTemplateColumns: `1fr ${sidebarWidth}` }}>
          <main className={styles.mainColumn}>{children}</main>

          <aside className={styles.sidebarColumn}>{sidebarContent}</aside>
        </div>
      </div>
    </div>
  );
