...
export const StudentSubmissionItem: React.FC<StudentSubmissionItemProps> = ({
  ...
  const statusConfig: Partial<
    Record<SubmissionStatus, { label: string; className: string; dotClassName: string }>
  > = {
    'Turned in': {
      label: 'Здано',
      className: styles.statusTurnedIn,
      dotClassName: styles.dotTurnedIn,
    },
    Assigned: {
      label: 'Призначено',
      className: styles.statusAssigned,
      dotClassName: styles.dotAssigned,
    },
    Missing: {
      label: 'Протерміновано',
      className: styles.statusMissing,
      dotClassName: styles.dotMissing,
    },
  };
  ...
  return (
    <div className={`${styles.container} ${isActive ? styles.active : ''}`} onClick={onClick}>
      ...
      <div className={styles.rightSection}>
        {status === 'Graded' ? (
          <p className={styles.gradeText}>{grade !== undefined && grade !== null ? grade : '—'}</p>
        ) : (
          currentStatus && (
