...
export const CourseInfoSection = ({ values, handlers, data = {} }: any) => {
    ...
    const teacherOptions = (data?.teachers || []).map((t: any) => ({ value: t.id, label: `${t.firstName} ${t.lastName}` }));
    const studentOptions = (data?.students || []).map((s: any) => ({ value: s.id, label: `${s.firstName} ${s.lastName}` }));
