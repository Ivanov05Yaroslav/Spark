...
export const useRoleManagement = () => {
    ...
    const loadUsers = useCallback(async (search: string, targetPage: number) => {
        ...
        try {
            ...
            const serverErrorMessage = error?.response?.data?.message || 'Не вдалося завантажити список користувачів';
    ...
    const handleRoleChange = useCallback((id: string, newRoles: string[]) => {
    ...
    const handleConfirmDelete = useCallback(async () => {
        ...
        try {
            ...
            const serverErrorMessage = error?.response?.data?.message || 'Не вдалося видалити користувача';
            toast.error(serverErrorMessage);
            loadUsers(searchQuery, 1);
        } finally {
            setIsDeleting(false);
