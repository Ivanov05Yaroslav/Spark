...
export const GeneralTaskDetailsPage: React.FC = () => {
    ...
    const [submissionStatus, setSubmissionStatus] = useState<'Assigned' | 'Turned in' | 'Graded' | 'Missing'>('Assigned');
    ...
    const [comments, setComments] = useState([
        {
            id: 'c1',
            author: { name: 'Дмитро Коваленко', avatarUrl: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSolfQ0VyDwW3mc6xHVFWn01UBMDpSXPY0wpw&s' },
            timestamp: 'Вчора, 18:40',
            content: "Підкажіть, будь ласка, чи можна використовувати власну палітру кольорів, чи обов'язково брати ту, що в ТЗ?"
        }
    ]);
    ...
    const handleRemoveFile = (fileId: string) => setSubmittedFiles(prev => prev.filter(f => f.id !== fileId));
    ...
    const handleAddComment = (content: string) => {
        setComments(prev => [
            ...
            {
                id: String(Date.now()),
                author: { name: 'Поточний Користувач', avatarUrl: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSolfQ0VyDwW3mc6xHVFWn01UBMDpSXPY0wpw&s' },
                timestamp: 'Щойно',
                content: content
            }
        ]);
    ...
    if (isLoading) {
        return <div style={{ display: 'flex', justifyContent: 'center', padding: '50px' }}>Завантаження сторінки завдання...</div>;
    }

    if (error || !task) {
        return <div style={{ display: 'flex', justifyContent: 'center', padding: '50px', color: 'red' }}>{error || 'Завдання не знайдено.'}</div>;
    }
