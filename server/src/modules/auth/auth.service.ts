...
export class AuthService {
  ...
  async initSchoolRegistration(dto: InitSchoolRegistrationDto) {
    ...
    if (!edeboSchool.directorFullName) {
      throw new HttpException('У реєстрі відсутнє ПІБ директора. Реєстрація неможлива.', HttpStatus.BAD_REQUEST);
    }
    ...
    if (existingSchool) {
      throw new HttpException('Цей навчальний заклад вже зареєстровано на платформі', HttpStatus.CONFLICT);
    }
  ...
  async processDiiaCallback(dto: DiiaCallbackDto) {
    ...
    try {
      ...
      const edeboNameNormalized = session.directorFullNameEdebo.toLowerCase().replace(/[^а-яіїєґa-z]/g, '');
      ...
      if (!edeboNameNormalized.includes(diiaNameNormalized)) {
        this.logger.warn(`Спроба реєстрації з невідповідним ПІБ. ЄДЕБО: ${session.directorFullNameEdebo}, Дія: ${diiaData.fullName}`);
  ...
  async sendSchoolRegistrationEmailCode(dto: SendSchoolEmailCodeDto) {
    ...
    if (!session || (session.status !== 'DIIA_VERIFIED' && session.status !== 'EMAIL_SENT')) {
      throw new HttpException('Невалідна сесія або ви не пройшли верифікацію Дії', HttpStatus.FORBIDDEN);
    }
  ...
  async verifySchoolRegistrationEmailCode(dto: VerifySchoolEmailCodeDto) {
    ...
    if (!session || session.status !== 'EMAIL_SENT') {
      throw new HttpException('Сесію не знайдено або код ще не відправлено', HttpStatus.BAD_REQUEST);
    }
    ...
    const edeboSchool = await this.prisma.edeboSchool.findUnique({ where: { edeboId: session.edeboId } });
    if (!edeboSchool) throw new HttpException('Помилка: школу не знайдено в базі', HttpStatus.INTERNAL_SERVER_ERROR);

    const nameParts = session.directorFullNameEdebo.replace(/(Директор|В\.о\.|Керівник)/gi, '').trim().split(' ');
